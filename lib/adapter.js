    ...
    var adapter = {
        ...
        drop: function(connection, collection, relations, cb) {
            ...
            grabConnection(connection).destroy(collection, {}, function(err, data) {
                // console.log('adapter.destroy data'.yellow, data);
                log('DROP', {
                    'CONNECTION': connection,
                ...
                if (err) {
                    ...
                    grabConnection(connection).drop(function(err, data) {
                        return cb(false, {});
        ...
        count: function(connection, collection, options, cb) {
            ...
            // console.log('Solr.adapter count'.magenta.inverse, options);
            // console.log('Solr adapter count options'.red.inverse, options);

            // /* model access */
            var model = connections[connection].collections[collection];

            if (model.queryParams.fq) {
                options = _.assign(options, {
                    fq: model.queryParams.fq
                });
            }
            // console.log('Solr adapter count options'.red.inverse, options);
            // if(model.queryParams.fq) {
            ...
            //     options.fq = [];
            //   }
            //   options.fq.push(model.queryParams.fq);
            // }
            // console.log('Solr adapter MODEL ACCESS'.red.inverse, options);
            // /* get additional query params*/
            // if(typeof model.getQueryParams === 'function') {
            //    // console.log('Solr adapter MODEL ACCESS'.red.inverse, model.getQueryParams());
            ...
            //         options.fq = [];
            //       }
            //       options.fq.push(model.getQueryParams('fq'));
            //    } else {
            //       options.fq = [model.getQueryParams('fq')];
            //    }
            // }
            ...
            grabConnection(connection).count(connection, options, function(err, data) {
                // sails.log.silly('Solr adapter count', err, data);
                if (err) {
        ...
        update: function(connection, collection, options, values, cb) {
            ...
            // console.log('Solr.adapter update'.cyan.inverse);

            // console.log(connection, collection, options, values);
        ...
        destroy: function(connection, collection, options, cb) {
            ...
            // console.log('Solr.adapter destroy'.cyan.inverse,options);
            ...
            //     options.fq = [];
            //   }
            //   options.fq.push(model.queryParams.fq[0]);

            //   // = _.assign(options, {fq:model.queryParams.fq});
            // }

            // if(options.where.hasOwnProperty('id') && options.where.id === null) {
            //   options.where.id='*';
            // }
            // console.log('DESTROY'.rainbow.inverse, options);
            // return cb(false, {});

            grabConnection(connection).destroy(connection, options, function(err, data) {
                // console.log('adapter.destroy data'.yellow, data, err);
                if (err) {
        ...
        // delete: function (connection, collection, options, values, cb) {

        //   var model = connections[connection].collections[collection];

        //   /* get additional query params*/
        //   // if(typeof model.getQueryParams === 'function') {
        //   //   options.queryParams = {fq: model.getQueryParams('fq')};
        //   // }

        //   console.log('DELETE'.red.inverse, arguments);

        //   return cb(false, {});
        //   grabConnection(connection,collection).destroy(connection, options,  function(err, data){
        //       console.log('adapter.delete data'.yellow, data);
        //     if(err) {
        //       console.log('adapter.delete error'.red, err);
        //       return cb(err, false);
        //     } else {
        //       if(data.responseHeader.status === 0) {
        //         return cb(values);
