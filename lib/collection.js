...
Collection.prototype.create = function(connection, collection, values, cb) {
    ...
    async.waterfall([
            ...
            function(validationError, documents, callback) {
                // console.log('validationError'.rainbow.inverse, validationError);
        ...
        function(err, validationError, createError, documents) {
            // console.log('CREATE STATE'.magenta.inverse, utils.inspect(arguments, 1));
            console.log('CREATE STATE validationError'.cyan.inverse, validationError);
            console.log('CREATE STATE createError'.cyan.inverse, createError);
            if (err) {
                return cb(true, false);
            } else if (validationError != false) {
                return cb(true, false);
            } else if (createError != false) {
                return cb(true, false);
            } else {
...
Collection.prototype._validateDocuments = function(connection, collection, values, cb) {
    ...
    // TODO: check error in array !!!!!????
    // TODO: check unique also!!!
    // return cb(false, docs);
    if (!model.primaryKey) {
        return cb(false, docs);
    }
    console.log('Collection._validateDocuments definitions: ', model.parsedDefinition);
    ...
    async.forEachOf(docs, function(doc, i, callback) {
        ...
        query = self.addDocModel(collection, query);

        if (model.primaryKey && !_.isUndefined(doc[model.primaryKey])) {
            query.where[model.primaryKey] = doc[model.primaryKey];
        } else {
            // console.log('definition'.green.inverse, model.definition);
            return callback();
        }

        // console.log('QUERY'.green.inverse, query);
        self.count(connection, collection, query, function(err, count) {
            console.log('COUINT ??? ? ? ? ? '.white.inverse, err, count, query);
            if (err) {
                console.log('Collection._validateDocuments error: ', err);
                return callback();
            }
            if (count !== 0) {
                errorDocs.push({
...
Collection.prototype._define = function(collection, definition, cb) {
    ...
    _.each(self.collections[collection].definition, function(field) {
        if (field.type == 'json') {
            parsedDefinition.jsonFields.push(field.columnsName || field.name);
        }
        if (field.type == 'binary') {
            parsedDefinition.binaryFields.push(field.columnsName || field.name);
        }
        if (field.type == 'date' || field.type == 'datetime') {
            parsedDefinition.dateFields.push(field.columnsName || field.name);
        }
        if (!_.isUndefined(field.unique) && field.unique == true) {
            parsedDefinition.unique.push(field.columnsName || field.name);
        }
    });
...
Collection.prototype.addAttribute = function(collection, field, cb) {
    ...
    if (1 == 1 || _.isUndefined(schema[field.name])) {
        ...
        // console.log('addAttribute'.cyan.inverse, addField);
