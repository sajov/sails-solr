...
Collection.prototype.describe = function(collection, cb) {
    ...
    // console.log('DESCRIBE collection'.magenta, collection, migrate);
...
Collection.prototype.defineOLD = function(collection, definition, cb) {
    var self = this;
    console.log('DEFINE old collection'.magenta, collection, self.collections[collection].migrate, definition);
    // console.log('DEFINE collection'.magenta, self.collections[collection]);
    // get schema
    self._getSchema(collection, function(err, schema) {
        if (err) {
            cb(err, false);
        } else {
