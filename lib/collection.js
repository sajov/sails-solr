...
Collection.prototype.find = function(connection, collection, options, cb) {
    ...
    options = self.addDocModel(collection, options);
    var query = new Query(options);
    log('Collection.prototype.find query'.red.inverse, query)
    query = query.queryUri;
...
Collection.prototype.count = function(collection, options, cb) {
...
Collection.prototype.getDocModel = function(connection, collection) {
    ...
    if (!_.has(self.collections[collection], 'schema.model_s')) {
        // return false;
    }
...
Collection.prototype.drop = function(connection, collection, cb) {
    ...
    async.waterfall([
            ...
            function(callback) {
                ...
                console.log('migrate???? '.magenta.inverse, {
                    collection: collection,
                    migrate: migrate
                });
...
Collection.prototype.describe = function(collection, cb) {
    var self = this;
    var migrate = self.collections[collection].migrate;
    log('DESCRIBE collection'.magenta, collection, migrate, self.collections[collection].autoCreatedAt);
...
Collection.prototype.define = function(collection, definition, cb) {
    var self = this;
    var migrate = self.collections[collection].migrate;
    log('DEFINE collection'.magenta, collection, self.collections[collection].migrate, definition);
...
Collection.prototype.addAttribute = function(collection, field, cb) {
    var self = this;
    log('addAttribute', collection, inspect(field));
    ...
    if (1 == 1 || _.isUndefined(schema[field.name])) {
        ...
        log('already defined field!!!', field);
        cb();
    }
}
