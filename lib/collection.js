...
Collection.prototype.select = function(options, defaultQuery, cb) {
    ...
    var QB = new Query(options, defaultQuery);
    // console.log('Collection.select buildQuery: '.cyan.inverse, QB.buildQuery());
    ...
    // console.log('Collection.select buildQuery: '.cyan.inverse, util.inspect(QB.buildQuery(), false,10,true));
    self.client.select(QB.buildQuery(), cb);
    // cb(false,{test:'test'});
}
...
Collection.prototype.get = function(options, defaultQuery, cb) {
    ...
    // console.log('Collection.select buildQueryString: '.cyan.inverse, (QB.buildQueryString()).cyan);
    // console.log('Collection.select buildQuery: '.cyan.inverse, util.inspect(QB.buildQuery(), false,10,true));
    self.client.realtime(QB.buildQuery(), cb);
    // cb(false,{test:'test'});
}
...
Collection.prototype.find = function(connection, collection, options, cb) {
    ...
    var QB = new Query(options, {});
    self.client.select(QB.buildQuery(), function(err, data) {
        if (err) {
            log('FIND error', err);
...
Collection.prototype._prepareDocuments = function(connection, collection, values) {
    ...
    var jsonFields = [];
    _.each(self.collections[connection].definition, function(field) {
        //JSON.parse
        if (field.type == 'json') {
...
Collection.prototype.getDocModel = function(connection, collection) {
    ...
    /* depends on count make no sence */
    // var modelCount = 0;
    // _.each(self.collections, function(coll, name) {
    //     log('CREATE', {
    //         name: name
    //     }, 2);
    //     modelCount++;
...
Collection.prototype.addDocModel = function(collection, options) {
    ...
    if (_.isUndefined(this.config.modelKey) || this.config.modelKey == false) {
        return options
    }
    if (_.isUndefined(options.fq)) {
        options.fq = [];
    }
    options.fq[this.config.modelKey] = collection;
...
Collection.prototype.destroy = function(collection, options, cb) {
    ...
    var QB = new Query(options);
    var queryObj = QB.buildQuery();

    //WTF user queryBuilder
    var query = queryObj.q;
    //TODO: what if object
    if (_.has(options, 'fq')) {
        query += ' AND ' + queryObj.fq.join(' AND ');
    }

    //add doc model if defined
    var docModel = self.getDocModel(collection);
    if (docModel) {
        query += ' AND ' + docModel.field + ':' + docModel.field;
    }

    console.log('Collection.destroy filter: '.cyan.inverse, query);
    ...
    self.client.deleteDocByQuery({
        ...
        // console.log('DESTROY QUERY'.rainbow,data,err);
        cb(false, options)
    });
...
Collection.prototype.drop = function(cb) {
    ...
    //         cb(err, false);
    //     } else {
    //         self._deleteFields(response.schema.fields, function(err, data) {
    //             log('DROP fields');
    //             if (err) {
    //                 cb(err, {});
    //             } else {
    //                 self._deleteDynamicFields(response.schema.dynamicFields, function(err, data) {
    //                     log('DROP dynamicFields');
    //                     if (err) {
    //                         cb(err, {});
    //                     } else {
    //                         cb(false, {});
...
Collection.prototype.describe = function(collection, cb) {
    ...
    self.client.getSchema(function(err, data) {
        if (err) {
            ...
            // log('DESCRIBE SCHEMAFIELDS RESPONSE', response);
            // self.schema = data.schema;
    ...
    self.describe(collection, function(err, schema) {
        if (err) {
            ...
            async.each(newFields, function(field, callback) {
                    ...
                    if (_.isUndefined(schema[field.name])) {
                        ...
                        // add default field attributes
                        field = _.assign(self.schemaDefaultFieldAttributes, field);

                        var addField = {};
                        // detect field dynamic
                        if (field.dynamicField == false) {
